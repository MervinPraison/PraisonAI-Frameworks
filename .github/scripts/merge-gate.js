...
const SDK_PATH_PREFIXES = ['src/praisonai_frameworks/'];
const BLOAT_FILES = ['src/praisonai_frameworks/base.py'];
...
const REQUIRED_SDK_CHECK_PATTERNS = [/test/i];
...
const FINAL_CLAUDE_REVIEW_BODY =
  '@claude You are the FINAL architecture reviewer. If the branch is under MervinPraison/PraisonAI-Frameworks (not a fork), you are able to make modifications to this branch and push directly. SCOPE: `src/praisonai_frameworks/`, `tests/`, `examples/`, `docs/` — optional framework adapters only. Read ALL comments above from Gemini, Qodo, CodeRabbit, and Copilot carefully before responding.\n\n**Phase 1: Review per AGENTS.md**\n1. Adapters belong here; core Agent APIs belong in PraisonAI\n2. Lazy imports and optional extras in pyproject.toml\n3. Backward compatible entry points\n4. Framework value: strengthens adapter layer without duplicating core SDK\n5. Do not bloat `base.py` — keep shared base minimal\n\n**Phase 2: FIX Valid Issues**\n6. For VALID bugs from reviewers: implement the fix\n7. Push fixes directly to THIS branch (do NOT create a new PR)\n8. Comment files modified and what you skipped\n\n**Phase 3: Final Verdict**\n9. If resolved, approve the PR; else request changes with clear action items';
