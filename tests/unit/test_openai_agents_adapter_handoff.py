"""OpenAI Agents adapter handoff unit tests."""
...
def test_handoff_to_resolves_by_role_string():
    ...
    def fake_build(role_key, details, llm_config, tools_dict, topic, tool_names=None):
        if role_key == "triage":
            return mock_triage
        return mock_english

    with patch.object(adapter, "_build_agent", side_effect=fake_build):
        by_key, by_role = adapter._build_agent_registry(config, [{"api_key": "k"}], {}, "topic")

    assert by_role["Triage Agent"] is mock_triage
    assert mock_triage.handoffs == [mock_english]
        ...
        "roles": {
            "triage": {
                ...
                "tasks": {
                    ...
                    "follow": {"description": "Second", "expected_output": "B", "context": ["route"]},
                },
            },
            "english": {
                "role": "English Agent",
